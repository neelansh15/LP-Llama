...
class LpRewardStore:
    ...
    def store_rewards(self):
        ...
        exchange = RuntimeExchange(w3, self.lp_exchange.address, self.lp_exchange.reward_address)
        date_gt = "2022-07-26"
        prev_df = self.get_df()
        if prev_df is not None:
            date_gt = prev_df.iloc[-1].name
        blocks = Block.objects.filter(chain_id=137, date__gt=date_gt).order_by("block_no")
        df_dict = {}
        for block in tqdm(blocks):
            reward, reward_price, lp_supply, lp_price, reserve0, reserve1 = exchange.get_lp_reward_details_on_block(self.lp.pool_id, block.block_no)
            df_dict[block.date] = {
                "reward": reward,
